...
<Card title="Gemini Flash 2.0" icon="bolt">
  ...
  in both chat and Command + K interfaces. Perfect for quick code completions and
  rapid prototyping.
</Card>

 <Card title="Grok Beta" icon="robot">
    A frontier language model with state-of-the-art reasoning capabilities. Suitable for chat, coding, and reasoning tasks.
  </Card>

<Card title="Perplexity" icon="searchengin">
  A conversational search engine providing real-time, sourced answers. While not
  tailored for coding, it excels at accessing coding documentation, examples,
...
<Note>
  The O1 model is limited to 50 credits. To purchase additional credits,
...
You can add your API keys directly in the Model Settings screen. Click the settings icon in the top right corner of the sidebar, and look for the API key input fields for each provider under the "Model Settings" section.
...
<Accordion title="Anthropic API Key">
  ...
  2. Find the "Anthropic Key" field.
  3. Enter your API key in the input box.
</Accordion>
...
<Steps>
  <Step title="Add Custom Model">
    1. Navigate to "Settings" by selecting the gear icon in the top right corner of the sidebar.
    2. Select "Models" to find the "Custom Models" section.
...
<Step title="Example: DeepSeek V3 Model">
  ...
  <Step title="Finalize Setup">
    Return to the chat screen by clicking the close icon in the top right
    corner of the sidebar. You will see the custom model in the model
    selection options.
  </Step>
</Steps>
...
Alex Sidebar supports running local AI models through Ollama, providing a free and privacy-focused alternative to cloud-based models. Here is an example of setting up a local powerful model like Qwen2.5-Coder:
...
<CardGroup cols={2}>
  <Card title="Model Selection Tips" icon="lightbulb">
    ...
    • Use Claude 3.5 Haiku or GPT-4 Mini for quick code completions.
  </Card>
