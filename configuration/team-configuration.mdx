...
Configure your team's AI models and manage member access through the Alex Sidebar Admin Portal. This allows organizations to standardize AI usage across all developers.
...
The **Advanced** tab provides additional controls:

### Telemetry Control

<Warning>
  Telemetry settings are only available for team accounts. Individual users cannot disable telemetry.
</Warning>

Teams can disable all telemetry including:
...
Configure which models are available to team members:
- Enable/disable specific model types
- Set default models for new members
- Override user preferences with team defaults
...
<AccordionGroup>
  <Accordion title="Models not appearing for team members">
    - Ensure members have synced their Alex Sidebar app
    - Check that models are saved (click "Save Changes")
